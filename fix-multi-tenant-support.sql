-- Fix Multi-Tenant Support with Domain-Based Organization Enforcement
-- This allows multiple independent tenants to be created
-- BUT: Each email domain can only have ONE organization/owner
-- Users from the same domain must join through invitations
-- This ensures proper multi-tenant SaaS architecture with domain-based organization grouping

-- Drop and recreate the check_tenant_exists function
...
BEGIN
  -- Always return false to allow multiple tenants
  RETURN QUERY
  SELECT 
    FALSE::BOOLEAN as tenant_exists,
...
-- Update handle_new_user to support domain-based multi-tenant with enforcement
...
-- Verify the changes
SELECT 'Multi-tenant support enabled successfully!' as status;
